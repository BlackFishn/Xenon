...
- **System & network monitor** — CPU, GPU, RAM, disks, throughput, ping/jitter, and real in-game FPS (PresentMon on Windows, MangoHud on Linux). Live graphs keep a rolling five-minute history across dashboard pages and System tabs while the app is visible; hiding the app pauses network sampling.
...
- **[PresentMon](https://github.com/GameTechDev/PresentMon)** — the standalone 2.5.1 reader measures the foreground or tracked game, including generated frames reported by the driver/SDK. Xenon averages all valid display durations over two seconds and keeps swap chains separate. Settings → Performance → Install PresentMon upgrades older readers; the verified download is stored alongside 1.x for rollback. Legacy 1.x display timing can miss Frame Generation, and the DWM fallback is not game-specific. Overlay counters may use different averaging windows.
