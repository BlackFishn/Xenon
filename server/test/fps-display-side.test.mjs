...
test('the foreground hint is wired without a module cycle', () => {
  assert.match(SERVER, /fpsMonitor\.setForegroundPid\(\(\) => gameDetect\.getForegroundPid\(\)\)/);
  assert.match(SERVER, /gameDetect\.setGameHint\(\(\) => fpsMonitor\.getGamingProcess\(\)\)/);
  const detector = readFileSync(new URL('../gamedetect.js', import.meta.url), 'utf8');
  assert.ok(!/require\(['"]\.\/fps(?:mon|-monitor)/.test(detector),
    'gamedetect receives its FPS hint by injection, so the fork import stays one-way');
});
...
test('the SDK reference documents both, and which one to draw', () => {
  ...
  assert.match(doc, /Draw `fps` unless you specifically want the\r?\n  difference/);
});
