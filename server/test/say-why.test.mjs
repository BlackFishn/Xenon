...
test('Twitch and YouTube get the same reason, and so does Spotify', () => {
  const src = read('../js/streaming-page.js');
  const code = cut(src, '  function genericLoginError(') + cut(src, '  function showDeviceLoginError(');
  let note = '';
  const showError = new Function('t', 'setNote', code + '\nreturn showDeviceLoginError;')(T, (_card, message) => { note = message; });
  for (const key of ['twitch', 'youtube']) {
    const button = { disabled: true };
    showError({ key }, { querySelectorAll: () => [] }, button, { error: 'http_500', detail: 'Backend unavailable' });
    assert.match(note, /http_500.*Backend unavailable/);
