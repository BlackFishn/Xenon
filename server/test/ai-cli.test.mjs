...
test('voice stays local and free: nothing quietly needs a Gemini key', () => {
  const S = read('../server.js');
  assert.match(S, /const useLocal = provider === 'ollama' \|\| provider === 'anthropic' \|\| subscriptionVoice \|\| aiCli\.isCliProvider\(provider\);/);
  ...
  assert.match(S, /\} else if \(sttProvider === 'ollama' \|\| sttProvider === 'anthropic' \|\| subscriptionVoice \|\| aiCli\.isCliProvider\(sttProvider\)\) \{/);
  assert.match(S, /if \(sttProvider === 'openai' && !subscriptionVoice\) \{[\s\S]{0,700}aiOpenai\.stt\(\{ apiKey: openaiKey, wavBuffer: wavData/);
