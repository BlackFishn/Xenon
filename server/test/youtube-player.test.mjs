...
function setup({ stored = '{}', denied = false, native = false } = {}) {
  const messages = [], events = {}, writes = [];
  let now = 10000;
  const stage = {
    firstChild: null,
    insertBefore(frame) { this.firstChild = frame; frame.parentNode = this; },
    removeChild(frame) { assert.equal(this.firstChild, frame); this.firstChild = null; frame.parentNode = null; },
  };
  const document = {
    querySelectorAll: () => [], querySelector: () => null,
    addEventListener() {}, body: { classList: { toggle() {}, remove() {} } },
  ...
  const instrumented = source.replace('window.YouTubeWidget = { renderWidgets, playFromSdk };',
    'window.YouTubeWidget = { parseVideoLink, normalizeVideo, player, saved, playList, playAt, playNext, applyState, toggleFavorite, stopPlayer, loadTab };');
  vm.runInContext(instrumented, context);
  return { ...window.YouTubeWidget, stage, messages, events, writes, advanceClock() { now += 2000; } };
}
