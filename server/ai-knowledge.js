...
const TOPICS = [
  ...
  },
  {
    ...
    keywords: ['ai', 'assistant', 'assistente', 'gemini', 'claude', 'anthropic', 'openai', 'ollama', 'claude code', 'codex', 'abbonamento', 'subscription', 'local', 'locale', 'api key', 'chiave', 'voice', 'voce', 'wake word', 'hey xenon', 'whisper', 'tts', 'genesis', 'guardian', 'pc control', 'controllo pc', 'memoria', 'memory', 'live', 'nircmd', 'screen vision'],
    body: 'Xenon AI runs on the provider chosen in Settings → Xenon AI: Google Gemini (default), Anthropic Claude, OpenAI, the user\'s own Claude or ChatGPT subscription through the official Claude Code or Codex program installed on the PC (no API key; same dashboard tools, a few seconds slower per answer), or a fully local stack (Ollama for chat, Whisper.cpp for speech-to-text, Edge neural voices for speech) — cloud providers need an API key, the local stack needs no internet for chat. Voice: tap the mic, or enable the local "Hey Xenon" wake word; Voce Live (opt-in) is full-duplex realtime voice on Gemini. Screen vision needs the small NirCmd utility (the installer fetches it). Opt-in extras in Settings → Funzioni AI: Genesis (composes dashboard pages), Guardian (long-term hardware-health history), PC Control (proposes Windows commands that run ONLY after you approve each one on a confirmation card), advanced reasoning, and persistent memory (on by default, stored locally, can be turned off). State-changing AI actions offer an Undo chip.',
  },
  {
    id: 'marketplace',
