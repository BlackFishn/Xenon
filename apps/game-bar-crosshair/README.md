...
The backend discovers one exact Xenon package identity in the current user’s Windows package storage. It rejects redirected storage and exchanges bounded JSON files in the package’s LocalState directory. Commands have unique IDs and expiry times; the widget validates them again. Writes use temporary files and rename. The widget checks for command-file changes every 50 ms while visible, reads only changed commands, and publishes a heartbeat every two seconds. The backend checks acknowledgements every 25 ms. The open editor refreshes status every 250 ms; the side-menu indicator uses a one-second interval. Hidden pages do not poll. Drag edits are coalesced without waiting for the drag to stop; at most one command is in flight, and its newest pending replacement is sent immediately after acknowledgement.
