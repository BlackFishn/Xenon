...
- **Claude Code** and **Codex** *(your subscription, no API key)* — Xenon AI answers through the official Claude Code or Codex program installed on your PC, signed in with your own Claude or ChatGPT plan. Xenon never sees or stores your credentials; Settings shows whether the program is installed and signed in, and what to run if it isn't, and lets you pick the model the program offers your account. The assistant keeps all its dashboard tools, which reach the program through a bridge that lives for one answer, while the program's own terminal and file tools stay off. Voice uses the free local speech, and Bit's automatic one-liners never spend your plan.
