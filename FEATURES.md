...
- Add a dedicated **Weather tile** to the dashboard from the **"+" → Productivity** palette — the same live card as the full panel (animated sky, location, temperature, condition, feels-like/wind/precipitation), **resizable** to any shape. It can also show the **same detail cards, hourly timeline and multi-day forecast** below the card, and you **choose which sections to show** per-widget in Settings → Weather (all off = clean current-conditions card; on = a full scrolling weather board). If the card takes more room than you want to give it, set **Current conditions → One line** in Settings → Weather: the card becomes a single row (icon, temperature, condition, and feels-like/wind/precipitation on the right) and the hours and days get the rest of the tile. The row stays tappable, and on a narrow tile it keeps feels-like to the last. Tap the card to open the full modal. Shares the top-bar chip's live data, city and °C/°F setting, and persists like any widget
