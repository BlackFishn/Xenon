...
The editor preview stays visible while the game overlay is OFF. Its label distinguishes the preview from the confirmed game state. Offline edits wait until you press ON or reconnect. ON launches Game Bar when needed, waits for the widget, applies any pending design and reports ON only after acknowledgement. Repeated clicks do not start duplicate launches. If Windows cannot open the widget, the editor shows an error and lets you retry; opening the panel or switching OFF never launches Game Bar.
